...
Core idea: AI products differ by where your context lives, which workflow they optimize, what controls they expose, and how billing behaves.
...
The scene compares alternatives through product fit rather than a single best-model ranking. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "The question is not only which model is best; it is which product fits the job." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "AI products differ by where your context lives, which workflow they optimize, what controls they expose, and how billing behaves." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: job -> context -> surface -> controls -> billing. Dots move through the graph to make the causal flow visible. | "Rovo, Gemini, Copilot, and Claude differ mainly in product context and harness defaults." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "The right AI product is the one whose harness fits the job." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "Exact plan pricing changes; verify vendor pages before recording. End of pack: use the same frame to evaluate new AI products." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `AIAlternatives` in `scene.py`.
