...
Core idea: An agent is a system that uses a model to choose actions, use tools, observe results, and continue toward a goal.
...
The scene distinguishes a single model response from an agentic system that uses tools, observations, and stop conditions. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "An agent is not just a model response; it pursues a goal through repeated decisions." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "An agent is a system that uses a model to choose actions, use tools, observe results, and continue toward a goal." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: goal -> context -> choose -> act -> observe. Dots move through the graph to make the causal flow visible. | "Each turn decides whether to use another tool or produce the final answer." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "An agent is the loop around model decisions, tool use, and observations." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "Long-horizon autonomy can trade latency and cost for task performance. Next: guardrails show how to constrain that loop." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `WhatIsAnAgent` in `scene.py`.
