...
Core idea: A harness hook is an event-triggered interception point that can log, validate, approve, deny, or transform behavior.
...
The scene explains hooks as lifecycle interception points that receive structured context and can enforce policy. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "A hook is not just logging; it can intercept behavior at lifecycle events." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "A harness hook is an event-triggered interception point that can log, validate, approve, deny, or transform behavior." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: event -> payload -> hook -> decision -> continue. Dots move through the graph to make the causal flow visible. | "Hooks attach to points such as session start, prompt submitted, pre-tool use, post-tool use, and session end." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "Hooks make harness policy executable at lifecycle boundaries." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "A hook that fails open can create false confidence. Next: plugins package hooks and other customizations." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `WhatIsAHarnessHook` in `scene.py`.
