...
Core idea: MCP is an open standard that lets AI hosts connect to external data sources, tools, and workflows through clients and servers.
...
The scene teaches MCP as a connector architecture with hosts, clients, servers, tools, data, and auth considerations. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "MCP is not one tool; it is a standard way for AI apps to connect to many tools." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "MCP is an open standard that lets AI hosts connect to external data sources, tools, and workflows through clients and servers." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: host -> client -> protocol -> server -> tool/data. Dots move through the graph to make the causal flow visible. | "The protocol separates the AI app from the external system it wants to use." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "MCP standardizes how AI applications reach external systems." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "A standard connector still needs permissions and monitoring. Next: alternatives show why product fit still matters." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `WhatIsAnMCP` in `scene.py`.
