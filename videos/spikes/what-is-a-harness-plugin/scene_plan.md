...
Core idea: A harness plugin is an installable package that bundles reusable agents, skills, hooks, integrations, or MCP configuration.
...
The scene explains a plugin as a shareable installable customization unit for harness behavior. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "A plugin is the packaging layer, not the model itself." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "A harness plugin is an installable package that bundles reusable agents, skills, hooks, integrations, or MCP configuration." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: package -> install -> unpack -> configure -> reuse. Dots move through the graph to make the causal flow visible. | "A plugin turns customization into a shareable, versioned unit." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "Plugins distribute harness behavior as a repeatable package." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "Installable convenience still needs governance and review. Next: skills are the reusable procedures inside many plugins." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `WhatIsAHarnessPlugin` in `scene.py`.
