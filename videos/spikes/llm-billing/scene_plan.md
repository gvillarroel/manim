...
Core idea: LLM cost is shaped by tokens, model tier, context size, retries, caching, and local hardware.
...
The scene avoids brittle price tables and teaches the durable billing model: tokens, model tier, context, retries, caching, and hardware. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "The invoice is not just a subscription: usage still follows tokens, model choice, and retries." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "LLM cost is shaped by tokens, model tier, context size, retries, caching, and local hardware." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: input -> model -> tools -> output -> meter. Dots move through the graph to make the causal flow visible. | "More context, stronger models, tool turns, and retries usually increase the bill." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "The fastest cost win is fewer wasted tokens and fewer bad turns." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "Local models are not free; hardware time and maintenance still count. Next: probabilities explain why retries can help and why they cost more." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `LLMBilling` in `scene.py`.
