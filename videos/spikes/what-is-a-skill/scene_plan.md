...
Core idea: A skill is a reusable folder of instructions, scripts, and resources that an AI system can load for a specialized task.
...
The scene defines skills as reusable on-demand procedure bundles with instructions and supporting files. The plan uses short claims only and pushes source detail into `research.md`.
...
| --- | --- | --- | --- | --- |
| 0:00-0:04 | Hook | Misconception text is crossed out while concept keywords gather below it. | "A skill is not a long prompt pasted forever; it is an on-demand capability bundle." | Title appears, the misconception is struck through, then keywords pulse into view. |
| 0:04-0:12 | Core definition | Definition becomes an equation-like strip using the recap terms. Three small annotations point to the same concept from different angles. | "A skill is a reusable folder of instructions, scripts, and resources that an AI system can load for a specialized task." | The hook transforms into the symbolic model and each term pulses in sequence. |
| 0:12-0:22 | Mechanism | A node graph appears: task -> trigger -> load -> execute -> finish. Dots move through the graph to make the causal flow visible. | "Skills keep specialized procedure out of the main context until it is useful." | Particles traverse the arrows, then input/rule/result callouts appear below the flow. |
| 0:22-0:31 | Practical implication | The weak frame visually transforms into the stronger frame; meters and a pressure curve grow beside it. | "A skill is reusable know-how with a trigger and supporting files." | The mechanism condenses into a contrast transform and animated consequence meters. |
| 0:31-0:39 | Cost, caution, handoff | A central concept node connects to recap terms in an orbit map while caution and next-step callouts appear. | "Bad skills can amplify bad process across many tasks. Next: MCP connects skills and agents to external systems." | Recap edges draw outward, terms pulse, then the scene fades. |
...
- Generate one Manim `Scene` subclass named `WhatIsASkill` in `scene.py`.
